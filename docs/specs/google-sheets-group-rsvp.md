...
5. При `Нет` поля алкоголя и транспорта скрываются и очищаются.
6. При `Да` или `Пока не знаю` гость заполняет транспорт и алкоголь.
7. Если гость приглашен на второй день, карточка показывает дополнительный блок:
   присутствие на втором дне, ночевка и алкоголь второго дня.
8. Одна кнопка сохраняет ответы всех гостей группы.
...
- `phone`: сохраненная колонка для существующих или ручных данных; сайт больше не читает и не обновляет это поле;
